...
####=================================Gram-Schmidt================================#
"""
    The function `modified_gram_schmidt` implements the modified Gram-Schmidt algorithm for QR
    decomposition.
    
    :param A: A is a matrix of shape (m, n) that you input into the modified Gram-Schmidt algorithm. The
    function `modified_gram_schmidt` takes this matrix A and performs the modified Gram-Schmidt
    orthogonalization process to compute the matrices Q and R such that A = QR
    :return: The `modified_gram_schmidt` function returns two matrices, `Q` and `R`. `Q` is an
    orthogonal matrix where the columns form an orthonormal basis for the column space of the input
    matrix `A`. `R` is an upper triangular matrix that represents the coefficients of the orthogonal
    projection of `A` onto the columns of `Q`.
    """
...
######===========================Time test====================########
def pruebas(case, number):
    if case ==0:
        matrix_ =np.random.rand(number,number)
    else:
        matrix_ =np.random.rand(2*number,number)
    print("====================QR with house holder============================")
    init = tm.perf_counter()
    q,r=householder_qr(matrix_)
    end = tm.perf_counter()
    print("Q:\n", q)
    print("R:\n", r)
    print("QR\n", q @ r)
    ...
    print("====================QR recursive blocks============================")
    init = tm.perf_counter()
    q,r=recursive_block_qr(matrix_,matrix_.shape[1], 50)
    end = tm.perf_counter()
    #print("Q:\n", q)
    #print("R:\n", r)
    print("QR\n", q @ r)
    #print("Original Mat: \n", matrix_)
    print("time elapsed: ", end-init)
    print("====================QR with GIVENS============================")
    init = tm.perf_counter()
    q,r=givens_QR(matrix_)
    end = tm.perf_counter()
    #print("Q:\n", q)
    #print("R:\n", r)
    print("QR\n", q @ r)
    #print("Original Mat: \n", matrix_)
    print("time elapsed: ", end-init)
    print("====================QR with Gram Schmidt Modified============================")
    init = tm.perf_counter()
    q,r=modified_gram_schmidt(matrix_)
    end = tm.perf_counter()
    #print("Q:\n", q)
    #print("R:\n", r)
    print("QR\n", q @ r)
    ...
    print("====================QR with Numpy============================")
    init = tm.perf_counter()
    q,r = np.linalg.qr(matrix_, 'complete')
    end = tm.perf_counter()
    #print("Q:\n", q)
    #print("R:\n", r)
    print("QR\n", q @ r)
...
nb = 10  # Tamaño del bloque
q,r = np.linalg.qr(A, 'complete')
Q, R = recursive_block_qr(A,A.shape[1], nb)
#print("Q:\n", Q)
#print("R:\n", R)
print("QR\n", Q @ R)
print("Matrix\n", A)
